...
Original file is located at
    https://colab.research.google.com/drive/1eDqHGBPr2yZdBQhhrcxOYpvsLm96DUcz
...
for x in random.sample(train_dataset, 1):
        print (len(x))
        print(x[0].split())

for x in random.sample(test_dataset, 1):
        print (x)
...
class UnigramModel(LanguageModel):
    ...
    def generateSentence(self):
        ...
        #return []
        sentence = []
        #random.sample(trainCorpus, 1)
        word = START
        while word != END:
              sentence.append(word)
              #word = choice(list(self.count_table.elements()))
              word=self.draw()
        sentence.append(END)
        return sentence        
...
class BigramModel(LanguageModel):
    ...
    def getSentenceLogProbability(self, sentence):
        ...
        prob=1.0
        for word in sentence[1:]:
            #prob*= math.log(self.prob(word, prior))
            prob*=self.prob(word, prior)
            prior=word
        #print(prob)
        #math.log(self.prob(word, prev)) if x != 0 else -inf
        return math.log(prob) if prob != 0 else -math.inf
    ...
    def getCorpusPerplexity(self, testCorpus):
        ...
        log_sum = 0.0
        for word in words:
#            log_sum += math.log(self.prob(word, prior)) 
            #print(math.log(self.prob(word, prior)))
            #print(self.prob(word, prior))
            #log_sum += (math.log(self.prob(word, prior)) if self.prob(word, prior) != 0 else math.inf)
#            log_sum += (math.log(self.prob(word, prior)) if self.prob(word, prior) != 0 else math.inf)
#            word_len+=1
    ...
    def train(self, corpus):
        for sentence in corpus:
            for i in range(len(sentence)-1):
                j=i+1
                self.priors[sentence[i]][sentence[j]]+=1
                self.total[sentence[i]]+=1

    # Returns the probability of word in the distribution
    def prob1(self, word, prior):
        result = self.priors[prior][word]/self.total[prior]
        return math.log(result) if result != 0 else -math.inf

    def prob(self, word, prior):
        #numerator = self.priors[prior][word]
        #denominator = self.total[prior] 
        return self.priors[prior][word]/self.total[prior]
        #return (self.priors[prior][word]/self.total[prior] if self.total[prior] != 0 else math.inf) 
...
#    addTransition(self, inStateName, inString, outString, outStateName): 
#   Add a transition between state inStateName and state outStateName, where both of these states already exist in the FST. 
#    The FST can traverse this transition after reading inString, and outputs outString when it does so.

#addSetTransition(self, inStateName, inStringSet, outStateName): 
#Add a transition between state inStateName and state outStateName for each character in inStringSet. 
#For each transition, the FST outputs the same character it reads.
  ...
  #  f.addSetTransition('q_OE', OE, 'q_ing')
